...
! Variable declaration
      ...
      real*4 :: d6,d7,psummc2,lsagejunk,lseratejunk
      real*8 :: randflt
      ...
      integer :: paminmc2,pamaxmc2,pnummc2,cnt8,curbasin,mc_iter_max
      integer*8 :: eratesum,eratechk,rint,peratesum,lseratesum
      logical lsero,datacomp,mcboth
      character :: buffer*8,obasin*12,pbasin*12,jc*5,hc*5,dump*80,mcschar*5
      character :: simyr*8
...
! Read in basin info
      ...
      do i=1,basnum                                                             ! Loop through number of subbasins to analyze
        ...
        !if (i.gt.0) then
          ...
          open(21,file='kmc_percent_pass_summary_'//trim(obasin)//'.dat',&
               status='unknown')      ! Open basin summary results file
    ...
    4     rewind (12)
          ...
          open (13,file='detrital_ages/erates_'//trim(pbasin)//'.dat',&
                status='old')
          do j=1,plc
            read(13,*) d1,d2,d3,perate(j),d4,d5                                 ! Read in erosion rates for scaling age distribution
...
!                    'erosion rates...'
!            open(14,file='detrital_ages/ls_ages_'//pbasin//'.dat',status='old')
!    5       read(14,*,end=6) dump
!            lsc=lsc+1
!            goto 5
!    6       rewind (14)
!            if (lsc.eq.0) then
!              print *,'*** Warning: No landslide ages found in given catchment ***'
!              print *,'Assigning junk age of',lsagejunk,'and erosion rate of',&
!                      lseratejunk
!              lsc=lsc+1
...
!          endif
           ...
           print *,'Landslide simulation requested, landslide files will be ',&
                        'read...'
...
!                        'erosion rates...'
    ...
    5           read(14,*,end=6) dump
                ...
                goto 5
    6           rewind (14)
                allocate(pagemc(lsc),pageumc(lsc))
                if (lsc.eq.0) then
                  !print *,'*** Warning: No landslide ages found in given catchment ***'
                  !print *,'Assigning junk age of',lsagejunk,'and erosion rate of',&
                  !        lseratejunk
...
! Uncomment the lines below if that is desired
              ...
              if (mcboth) then
                ...
                do k=1,mcsamp
                  ...
                  rint=int8(randflt*(peratesum))+1                              ! Get random integer value within range of size of scaled age dist.
                  eratechk=rint
                  cnt=0
                  do while (eratechk.gt.0)
                    cnt=cnt+1
                  ...
                  pagemc2(k)=page(cnt)                                          ! Add random age to monte carlo age array
                  pageumc2(k)=pageu(cnt)                                        ! Add associated uncertainty to monte carlo uncertainty array
                enddo
...
! Write out mc_iter monte carlo PDFs
            if (j.le.mc_iter_max) then
              write(jc,'(i5)') j
              if (j.lt.10) jc(1:4)='0000'
              if (j.lt.100) jc(1:3)='000'
              if (j.lt.1000) jc(1:2)='00'
              if (j.lt.10000) jc(1:1)='0'
              open(25,file='age_pdf_output/mc_age_PDF_'//jc//'_'&
                   //trim(obasin)//'.dat',status='unknown')
              do k=1,pnummc+1
