...
- **Value 2**: Predicted age file format. Type: ``int``
    ...
    **NOTE**: Values 3, 4, ... differ depending on the value selected for value 2 on this line.
    ...
    1. ``Comparison.txt`` file generated by `HUGG version of Pecube <https://github.com/HUGG/Pecube-HUGG>`_
        ...
        If you set Value 2 equal to 1, you should complete this line with the following values:

        - **Value 3**: Name of the Pecube model run output directory (e.g., ``RUN00``) in the ``data`` subdirectory
        - **Value 4**: The predicted thermochronometer age system to use (``1`` = AHe, ``2`` = AFT, ``3`` = ZHe, ``4`` = ZFT, ``5`` = MAr)
